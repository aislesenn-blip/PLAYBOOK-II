...
*   **Teacher Dashboard (Authenticated & Secure):**
    ...
    *   **Role:** The command center. Teachers authenticate here to create sessions (Assignments), configure Marking Schemes, store their AWS Textract and DeepSeek API Keys (BYOK) securely in the database, view Analytics, and review/override AI decisions.
...
To execute grading without exposing the Teacher's AWS and DeepSeek API keys to the student-facing frontend, we must route all AI requests through a secure backend proxy.
...
*   **The Flow:**
    ...
    5.  The Edge Function first uses the AWS SDK (Textract) to extract pure text from the PDF.
...
*   **Two-Step Pipeline:** The input is converted to text by AWS Textract, guaranteeing that DeepSeek only operates on pure text data for reasoning, eliminating vision hallucination risk and massively reducing token consumption.
