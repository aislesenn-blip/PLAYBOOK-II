...
*   **Teacher Dashboard (Authenticated & Secure):**
    ...
    *   **Role:** The command center. Teachers authenticate here to create sessions (Assignments), configure Marking Schemes, store their Google AI Studio API Key (BYOK) securely in the database, view Analytics, and review/override AI decisions.
...
To execute grading without exposing the Teacher's Google AI Studio API key to the student-facing frontend, we must route all AI requests through a secure backend proxy.
...
*   **The Flow:**
    ...
    5.  The Edge Function makes the HTTP call to Google AI Studio (`gpt-4o`).
    6.  The result is written back to the `submissions` table.
*   **Result:** The student's browser *never* touches the Google AI Studio API or the Teacher's API key.
...
Concurrency is great, but Google AI Studio rate limits (e.g., "Tier 1: 20 RPM") will reject 200 simultaneous calls from the same API key.
...
*   **The Strategy:**
    ...
    4.  **Exponential Backoff & Jitter:** If Google AI Studio returns a `429 Too Many Requests`, the worker catches the error, delays the job (e.g., waits 10 seconds, then 20, then 40), and safely retries without failing the student's submission.
*   **Result:** 200 students can submit at 11:59 PM. The system accepts all of them instantly (UI shows "Submitted - Grading in Progress"), but the queue strictly drips them to Google AI Studio at a safe rate of 5 at a time, protecting the Teacher's key.
...
*   **Temperature Control:** The Google AI Studio payload will remain hardcoded to `temperature: 0.0` and `top_p: 0.1` inside the Edge Function to prevent creative hallucinations.
