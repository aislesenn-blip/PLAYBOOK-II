...
async function fetchSiliconFlow(apiKey: string, systemPrompt: string, userContent: string, title: string) {
    let attempt = 0;
    while (attempt < 3) {
        try {
            ...
            attempt++;
            console.warn(`Appeal SiliconFlow attempt ${attempt} failed: ${error.message}`);
...
serve(async (req) => {
  ...
  try {
    ...
    // 4. Call AI
    const rawAIResponse = await fetchSiliconFlow(siliconFlowApiKey, APPEAL_SYSTEM_PROMPT, userPrompt, "Playbook Deep-Dive Appeal",);
